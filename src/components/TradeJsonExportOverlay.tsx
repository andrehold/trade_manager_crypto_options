    ...
    <Overlay open={open} onClose={onClose}>
      <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[80vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between border-b p-4">
          <div>
            <div className="text-sm text-slate-500">Export trade JSON</div>
        ...
        <div className="p-4 overflow-auto">
          ...
          {rows.length === 0 ? (
            <div className="text-sm text-slate-500">No legs available to export.</div>
          ) : (
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.id} className="border-b last:border-0">
                      <td className="p-2">
                        <input
                          className="w-full rounded-lg border border-slate-200 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
                      ...
                      <td className="p-2 text-right">
                        <input
                          ...
                          min={0}
                          step="0.1"
                          className="w-24 rounded-lg border border-slate-200 px-2 py-1 text-right font-mono text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
                          value={row.qty}
                          onChange={(e) =>
                            setRows((prev) =>
                              prev.map((item) =>
                      ...
                      <td className="p-2">
                        <select
                          className="rounded-lg border border-slate-200 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
                          value={row.side}
                          onChange={(e) =>
                            setRows((prev) =>
                              prev.map((item) =>
                      ...
                      <td className="p-2 text-right">
                        <button
                          type="button"
                          className="inline-flex items-center justify-center rounded-md border border-slate-200 bg-white p-2 text-slate-500 hover:bg-slate-100"
        ...
        <div className="border-t p-4 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow disabled:cursor-not-allowed disabled:opacity-50"
          >
