...
export function TradeJsonExportOverlay({ open, onClose, position, marks }: TradeJsonExportOverlayProps) {
  ...
  return (
    <Overlay open={open} onClose={onClose}>
      <div className="bg-surface-section rounded-2xl shadow-xl border border-border-default max-w-3xl w-full max-h-[80vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between border-b border-border-default p-4">
          <div>
            <div className="type-subhead text-faint">Export trade JSON</div>
            <div className="type-title-m font-semibold text-heading">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-2 text-body hover:bg-surface-hover"
        ...
        <div className="p-4 overflow-auto">
          <div className="type-subhead text-body mb-3">
            Review and edit legs before exporting the trade payload.
          </div>
          {rows.length === 0 ? (
            <div className="type-subhead text-faint">No legs available to export.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full type-subhead">
                <thead className="text-faint border-b border-border-default">
                  <tr>
                  ...
                  {rows.map((row) => (
                    <tr key={row.id} className="border-b border-border-default last:border-0">
                      ...
                      <td className="p-2 text-right">
                        <button
                          type="button"
                          className="inline-flex items-center justify-center rounded-md border border-border-strong bg-surface-card p-2 text-body hover:bg-surface-hover"
        ...
        <div className="border-t border-border-default p-4 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-surface-primary-btn px-4 py-2 type-subhead font-medium text-on-primary-btn shadow disabled:cursor-not-allowed disabled:opacity-50"
          >
