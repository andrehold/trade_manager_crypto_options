...
export function UploadBox({ onFiles }: { onFiles: (files: FileList) => void }) {
  ...
  return (
    <div className="border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center">
      <p className="type-subhead text-slate-600">Drop your Deribit CSV here, or</p>
      <button
        onClick={() => ref.current?.click()}
        className="mt-3 rounded-xl bg-slate-900 text-white px-4 py-2 type-subhead shadow"
      >Choose file</button>
