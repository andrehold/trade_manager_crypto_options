...
export function PremiumBadge({ value }: { value: number }) {
  ...
  const sign = isCredit ? '+' : '-'
  return (
    <span className="inline-flex items-center gap-1 bg-layer-chip rounded-md px-2 py-0.5 text-[11px] font-bold leading-tight text-zinc-100">
      {isCredit
...
>(function TradeCard(
  ...
  return (
    <div
      ref={ref}
      className={`flex flex-col bg-layer-card border border-zinc-700/60 rounded-xl px-3 py-2.5 select-none${className ? ' ' + className : ''}`}
      {...rest}
    >
      ...
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="type-caption font-bold text-zinc-100 truncate">{label}</span>
        </div>
        {onRemove && (
          <button
            onClick={(e) => { e.stopPropagation(); onRemove() }}
            className="shrink-0 text-zinc-600 hover:text-rose-400 text-[11px] leading-none ml-1 transition-colors"
      ...
      {expiryPart && (
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <span className="text-[11px] text-zinc-400 whitespace-nowrap">{expiryPart}</span>
        </div>
      )}
      ...
      {(datePart || timePart) && (
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-[11px] text-zinc-400 whitespace-nowrap" title={fullTimestamp}>
